...
export default function AdmissionPage() {
  ...
  const [localAdmissions, setLocalAdmissions] = useState(MOCK_ADMISSIONS.data)
  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newApplicant = {
      id: `APP-25-${(localAdmissions.length + 1).toString().padStart(3, '0')}`,
    ...
    setLocalAdmissions([newApplicant, ...localAdmissions])
            ...
            <div className="card animate-in stagger-2 p-0 overflow-hidden">
              <div className="table-wrapper">
                <table className="table">
                  ...
                  <tbody>
                    {displayData.map((row: typeof MOCK_ADMISSIONS['data'][0]) => (
                      <tr key={row.id}>
