...
export default function StudentsPage() {
  ...
  const { data = { data: localStudents, total: localStudents.length, last_page: Math.ceil(localStudents.length / itemsPerPage) } } = useQuery({
            ...
            <form onSubmit={handleUpdateStudent} className="p-8 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={editFormData.status} onChange={e => setEditFormData({...editFormData, status: e.target.value})} className="select w-full">
                    <option>Active</option><option>Inactive</option>
                  </select>
                </div>
              </div>
            ...
            <form onSubmit={handleAddStudent} className="p-8 space-y-5 overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={addFormData.gender} onChange={e => setAddFormData({...addFormData, gender: e.target.value})} className="select w-full">
